...
Since there are only 15 large districts and it appears that they have a different population mean for college attendance rates, we will exclude the big districts in our regression model. It is worth noting that since a large number of the state's students come from these larger districts, in the future it is worth analyzing these districts in more detail independent from the other districts. Before excluding the large districts, lets take a look at the college attendance rates for each group of interest in the big districts, to see if it differs from the group means when considering all districts. 
...
We can see that the differences within each group are roughly the same when utilizing means from all districts, however, each group mean is significantly lower when only using big districts compared to all districts. Now we can exclude large districts in our regression data set. Additionally, charter schools are not required to report many of the variables we will use in the regression model, and so we will also exclude them from our analysis. 


``` {r, message=FALSE}
data <- data %>% filter(DistSize != "Big")
...
The results of this initial model show that the adjusted R-squared value is slightly 60%, meaning that 60% of the variance with respect to college attendance rates can be attributed to these variables. 

``` {r, message=FALSE}
Anova(fitall)
```

We can use modified hypothesis testing to determine whether a variable is contributing to the model. Based on the results above, we can see that 
`needs_improv` and `enroll_hisp` are not heavily contributing to the model. We can remove these variables and refit the regression model. 

```{r, message=FALSE}
fitnew <- lm(college ~ sat_math + avg_salary + per_pupil + advanced_course + actualNSS + enroll_lowincome, data=collegeattendvars)
summary(fitnew) 
```
...
The plots show that 1. there is no distinctive residual patterns 2. residuals are normally distributed 3. residuals do not violate homoscedasity 4. there seem to be no hugely influential cases. The results of this regression analysis can be found in much more detail (and in plain english) on the results section of the project website. 